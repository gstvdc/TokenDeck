...
| `BOARD_HAS_IMU`                | 0       | Whether the accelerometer is populated and initialized. Distinct from `BOARD_HAS_ROTATION` — a board can have the IMU populated (`HAS_IMU=1`) while its enclosure mounts the panel at a fixed orientation, so rotation is off. |
...
| `BOARD_HAS_PWR_BUTTON`         | 0       | Whether a dedicated PWR key (PMU PKEY or an IO-expander line) exists to cycle screens/animations. When 0, `main.cpp`'s pwr-press handler and the pairing-screen text in `ui.cpp` instead treat a plain screen tap as the pairing action (see `cyd_28`, which has no PMU). |
